# Basic Demo (Text Mode) — Open Watcom + Open Zinc

A feature-rich demonstration application showcasing Open Zinc UI components and event handling for DOS/4GW 386 extended mode, in **text mode only** (no graphics).
...
All functionality works in 80x25 text mode without graphics.
...
This example uses Open Zinc's text-mode facilities exclusively (`UIW_*` classes without graphics drivers). It demonstrates that rich UI applications are possible in text mode on DOS.
